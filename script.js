    ...
    try {
        ...
        if (wantBackup) {
            try {
                ...
                const backupFileName = `FULL_BACKUP_ATCLMT_${currentYear}_${timestamp}.xlsx`;
...
// === BẮT ĐẦU: HÀM MỚI ĐỂ THÊM DÒNG VÀO MODAL ===
  ...
  window.openAddModal = async function(parentPath, level) {
    ...
    await loadEquipmentList();
        ...
        else if (level === 'child') title = 'Thêm lĩnh vực ATCLMT';
        else if (level === 'grandchild') title = 'Thêm đầu mục công việc';
        else if (level === 'greatGrandchild') title = 'Thêm công việc chi tiết';
 ...
 */
   window.generateWorksheetWithFormulas = async function(includeJustifications) {
    ...
    const baseHeaders = [
        ...
        'Cấp độ', 'CP Cấp 1', 'TG bắt đầu', 'TG hoàn thành', 'Ghi chú'
    ];
    // ---------------------------
    ...
    const colNameMap = {
        ...
        'TG bắt đầu': 'TG_BATDAU', 'TG hoàn thành': 'TG_HOANTHANH', 'Ghi chú': 'GHI_CHU'
    };
...
window.exportPlainDataExcel = async function() {
    try {
        ...
        const fileName = includeJustifications ? 
            `AT-CL-MT_${currentYear}_Full.xlsx` : 
            `AT-CL-MT_${currentYear}_NoJustification.xlsx`;
...
// === HÀM XUẤT EXCEL (CẬP NHẬT: EXCEL 1 ĐỦ CỘT NHƯNG KHÔNG GIẢI TRÌNH) ===
    ...
    try {
        ...
        const baseHeadersFull = ['TT', 'Nội dung công việc', 'Tag Name', 'ĐV th.hiện', 'KH năm trước', 'Thực hiện năm trước', 'Tần suất TH', 'Đ.vị tính', 'Số lượng', 'Đơn giá (GT)', 'Chi phí KH', 'Chi phí thực hiện', 'Kế hoạch còn lại', 'Năm phân bổ', 'CP phân bổ', 'Cấp độ', 'CP Cấp 1', 'TG bắt đầu', 'TG hoàn thành', 'Ghi chú'];
 ...
 window.fetchData = async function() {
    ...
    try {
        ...
        const getLevelSpecificActions = (level, path, data) => {
            if (level === 'parent') return `<a href="javascript:void(0)" onclick="openAddModal('${path}', 'child')">Thêm lĩnh vực ATCLMT</a>`;
            if (level === 'child') return `<a href="javascript:void(0)" onclick="openAddModal('${path}', 'grandchild')">Thêm mục công việc</a>`;
        ...
        const createRowHtml = (itemClass, path, data) => {
            ...
            // === BẮT ĐẦU ĐOẠN CODE đổi màu khi thực hiện CV ===
            // 1. Mặc định kiểm tra xem có chi phí thực hiện không
            let hasActiveExecution = data.chiPhiThucHien > 0;

            // 2. Nếu chưa có chi phí (chi phí = 0), kiểm tra tiếp xem có link hồ sơ nào không
            if (!hasActiveExecution && data.executions) {
                // Duyệt qua tất cả các lần thực hiện (node con trong executions)
                // Hàm .some() trả về true ngay lập tức nếu tìm thấy ít nhất 1 trường hợp thỏa mãn
                hasActiveExecution = Object.values(data.executions).some(exec => 
                    exec.hoSoLink && String(exec.hoSoLink).trim() !== ''
                );
            }

            // 3. Áp dụng style nếu 1 trong 2 điều kiện trên thỏa mãn
            const noiDungStyle = hasActiveExecution ? 'style="color: #5B77A8;"' : '';
            // === KẾT THÚC ĐOẠN CODE đổi màu ===
